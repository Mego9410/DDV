-- Add geo disclosure to ddv_query_intent:
--  * geo_unresolved: true when a "near" filter's center could not be located
--    (so callers can say "couldn't locate that place" instead of reporting 0).
--  * geo_missing: number of practices excluded from a radius search because they
--    have no coordinates (geog is null), so callers can disclose the exclusion.
--
-- Supersedes 20260602094200_add_radius_filter_ddv_query_intent.sql (function body
...
declare
  ...
  -- geo disclosure
  geo_used boolean := false;
  geo_unresolved boolean := false;
  geo_missing bigint := 0;
...
begin
  ...
  -- If a radius center could not be resolved, report that explicitly rather than
  -- returning a misleading 0.
  if geo_unresolved then
  ...
  -- Count practices that cannot participate in a radius search (no coordinates).
  if geo_used then
    select count(*) into geo_missing from public.practices where geog is null;
  end if;
